...
class GroupedUserDenseProjector(nn.Module):
    """Project user_dense by fid-aware branches and fuse back to one token."""

    GROUPS = {
        'ue': {61},
        'semantic': {87},
        'stats': {62, 63, 64, 65, 66},
        'small': {89, 90, 91},
    }
    ...
    def __init__(
        ...
        d_model: int,
        hidden_mult: int = 2,
        stats_clip: float = 20.0,
    ) -> None:
        super().__init__()
        self.stats_clip = float(stats_clip)
        self.group_slices = {name: [] for name in ['ue', 'semantic', 'stats', 'small', 'other']}
        for fid, offset, length in feature_specs:
            group_name = 'other'
            for name, fids in self.GROUPS.items():
                if int(fid) in fids:
                    group_name = name
                    break
            self.group_slices[group_name].append((int(offset), int(length)))

        self.group_names = [name for name, slices in self.group_slices.items() if slices]
        self.group_dims = {
            name: sum(length for _, length in self.group_slices[name])
            for name in self.group_names
        }
        hidden_dim = max(d_model, d_model * hidden_mult)
        self.group_mlps = nn.ModuleDict({
            name: nn.Sequential(
                nn.LayerNorm(self.group_dims[name]) if name == 'stats' else nn.Identity(),
                nn.Linear(self.group_dims[name], hidden_dim),
                nn.SiLU(),
        ...
        fusion_in = max(1, len(self.group_names)) * d_model
        self.gate = nn.Sequential(
            nn.Linear(fusion_in, max(d_model, fusion_in // 2)),
            nn.SiLU(),
            nn.Linear(max(d_model, fusion_in // 2), len(self.group_names)),
        )
    ...
    def _slice_group(self, x: torch.Tensor, name: str) -> torch.Tensor:
        parts = [x[:, offset:offset + length] for offset, length in self.group_slices[name]]
        out = torch.cat(parts, dim=-1)
        if name == 'stats':
            out = torch.sign(out) * torch.log1p(torch.abs(out))
            out = torch.clamp(out, -self.stats_clip, self.stats_clip)
        return out

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        group_tokens = []
        for name in self.group_names:
            group_x = self._slice_group(x, name)
            group_tokens.append(self.group_mlps[name](group_x))
        stacked = torch.stack(group_tokens, dim=1)
        cat = torch.cat(group_tokens, dim=-1)
        weights = torch.softmax(self.gate(cat), dim=-1).unsqueeze(-1)
        gated = (stacked * weights).sum(dim=1)
        return self.fuse(cat) + gated


class TimeContextProjector(nn.Module):
    """Project root/sequence time summary features to one d_model vector."""
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        if self.has_user_dense:
            if user_dense_projector_type == 'flat':
                self.user_dense_proj = nn.Sequential(
                    nn.Linear(user_dense_dim, d_model),
                    nn.LayerNorm(d_model),
                )
            elif user_dense_projector_type == 'grouped':
                if not user_dense_feature_specs:
                    raise ValueError(
                        "user_dense_feature_specs is required when "
                        "user_dense_projector_type='grouped'")
                self.user_dense_proj = GroupedUserDenseProjector(
                    user_dense_feature_specs, d_model=d_model)
            else:
                raise ValueError(
                    f"Unknown user_dense_projector_type: {user_dense_projector_type}")
            if self.use_time_context:
                self.time_context_proj = TimeContextProjector(
    ...
    def _init_params(self) -> None:
        ...
        if self.use_seq_time_delta_proj:
            nn.init.zeros_(self.time_delta_proj.weight.data)
            nn.init.zeros_(self.time_delta_proj.bias.data)
    ...
    def _make_user_dense_token(self, inputs: ModelInput) -> torch.Tensor:
        user_dense_tok = F.silu(self.user_dense_proj(inputs.user_dense_feats))
        if self.use_time_context and inputs.time_context is not None:
            time_ctx = inputs.time_context.to(
                device=user_dense_tok.device,
                dtype=user_dense_tok.dtype,
            )
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        ns_parts = [user_ns]
        if self.has_user_dense:
            ns_parts.append(self._make_user_dense_token(inputs))
