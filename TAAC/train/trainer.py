...
class PCVRHyFormerRankingTrainer:
    ...
    def _log_scalar_metrics(
        ...
        if log_to_file:
            formatted = ", ".join(
                f"{key}={value:.6g}" if np.isfinite(value) else f"{key}={value}"
    ...
    def train(self) -> None:
        ...
        for epoch in range(1, self.num_epochs + 1):
            ...
            for step, batch in train_pbar:
                loss, train_metrics = self._train_step(batch)
                total_step += 1
                loss_sum += loss
                metric_count += 1
                ...
                self._log_scalar_metrics('TrainStep', train_metrics, total_step)
            ...
            if epoch >= self.reinit_sparse_after_epoch and self.sparse_optimizer is not None:
                old_sparse_optimizer = self.sparse_optimizer
                reinit_ptrs = self.model.reinit_high_cardinality_params(
                    self.reinit_cardinality_threshold)

                # Preserve only states for params that survived reinit. In the
                # full-reinit setting this avoids holding the discarded Adagrad
                # accumulators for almost all embedding tables.
                old_state: Dict[int, Any] = {}
                for group in old_sparse_optimizer.param_groups:
                    for p in group['params']:
                        ptr = p.data_ptr()
                        if ptr not in reinit_ptrs and p in old_sparse_optimizer.state:
                            old_state[ptr] = old_sparse_optimizer.state[p]

                self.sparse_optimizer = None
                del old_sparse_optimizer
                torch.cuda.empty_cache()
                ...
                logging.info(f"Rebuilt Adagrad optimizer after epoch {epoch}, "
                             f"old_state candidates={len(old_state)}, "
                             f"reinit params={len(reinit_ptrs)}, "
    ...
    def _train_step(self, batch: Dict[str, Any]) -> Tuple[float, Dict[str, float]]:
        ...
        metrics = {
            ...
            'label_mean': float(label.detach().mean().item()),
            'prob_mean': float(probs.mean().item()),
            'prob_std': float(probs.std(unbiased=False).item()),
            'logit_mean': float(logits_for_metrics.mean().item()),
            'logit_std': float(logits_for_metrics.std(unbiased=False).item()),
            'grad_norm': float(grad_norm.item()),
            'dense_lr': float(self.dense_optimizer.param_groups[0]['lr']),
        }
        if self.sparse_optimizer is not None:
            metrics['sparse_lr'] = float(self.sparse_optimizer.param_groups[0]['lr'])
    ...
    def evaluate(self, epoch: Optional[int] = None) -> Tuple[float, float, Dict[str, float]]:
        ...
        metrics = {
            ...
            'num_samples': float(len(probs)),
            'nan_count': float(nan_mask.sum()),
        }
        if len(probs) > 0:
            metrics.update({
        ...
        if len(valid_logits) > 0:
            valid_logits_np = valid_logits.detach().cpu().numpy()
            metrics.update({
                'logit_mean': float(valid_logits_np.mean()),
                'logit_std': float(valid_logits_np.std()),
                'logit_min': float(valid_logits_np.min()),
                'logit_max': float(valid_logits_np.max()),
            })

        return auc, logloss, metrics
