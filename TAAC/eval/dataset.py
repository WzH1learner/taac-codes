...
def _get_row_group_timestamp_range(
    ...
    """Return min/max timestamp for a Row Group using metadata when possible."""
    if timestamp_col_idx < 0:
        return None, None

    row_group_meta = pf.metadata.row_group(row_group_idx)
    try:
        stats = row_group_meta.column(timestamp_col_idx).statistics
    ...
    try:
        table = pf.read_row_group(row_group_idx, columns=['timestamp'])
        values = table['timestamp'].to_numpy(zero_copy_only=False)
        if len(values) == 0:
            return None, None
...
def _log_rg_time_range(
    name: str,
    rg_info: List[Tuple[str, int, int, Optional[int], Optional[int]]],
    indices: List[int],
) -> None:
    """Log timestamp range for a selected set of Row Groups."""
    mins = [rg_info[idx][3] for idx in indices if rg_info[idx][3] is not None]
    maxs = [rg_info[idx][4] for idx in indices if rg_info[idx][4] is not None]
    if not mins or not maxs:
        logging.info(f"Row Group {name} timestamp range: unavailable")
        return
    logging.info(
        f"Row Group {name} timestamp range: "
        f"min={min(mins)}, max={max(maxs)}"
    )
...
class PCVRParquetDataset(IterableDataset):
    ...
    def __init__(
        ...
        Args:
            ...
            row_group_indices: explicit Row Group indices into the global
                file-order Row Group list. Mutually exclusive with
                ``row_group_range``. This is used for non-tail validation
                splits such as random or middle slices.
        ...
        if row_group_range is not None and row_group_indices is not None:
            raise ValueError("row_group_range and row_group_indices are mutually exclusive")

        if row_group_indices is not None:
            max_idx = len(self._rg_list) - 1
            bad = [idx for idx in row_group_indices if idx < 0 or idx > max_idx]
            if bad:
                raise IndexError(
                    f"row_group_indices contains out-of-range values {bad[:5]} "
                    f"for {len(self._rg_list)} Row Groups"
                )
            self._rg_list = [self._rg_list[idx] for idx in row_group_indices]
        ...
        if self.timestamp_min is not None or self.timestamp_max is not None:
            logging.info(
                "PCVRParquetDataset timestamp filter: min=%s inclusive, max=%s exclusive",
    ...
    def _filter_batch_by_timestamp(
        ...
        """Apply row-level timestamp filtering after conversion.

        RowGroup-level slicing is too coarse for the official test window,
        which can be only a few hours wide. This filter keeps the input
        construction unchanged while allowing sample-level time-window valid.
        """
        ...
        if self.timestamp_min is not None:
            mask &= timestamps >= int(self.timestamp_min)
        if self.timestamp_max is not None:
            mask &= timestamps < int(self.timestamp_max)

        if not bool(mask.any()):
            return None

        idx = torch.nonzero(mask, as_tuple=False).squeeze(1)
        n_rows = int(timestamps.shape[0])
        filtered: Dict[str, Any] = {}
        idx_list = idx.tolist()
        for k, v in batch.items():
            if k.startswith('_'):
                filtered[k] = v
            elif isinstance(v, torch.Tensor) and v.shape[0] == n_rows:
                filtered[k] = v[idx]
            elif isinstance(v, list) and len(v) == n_rows:
                filtered[k] = [v[i] for i in idx_list]
            else:
                filtered[k] = v
        return filtered
    ...
    def _flush_buffer(
        ...
        for i in range(0, total_rows, self.batch_size):
            ...
            idx_list = rand_idx[i:end].tolist()
            for k, values in row_list_keys.items():
                batch[k] = [values[j] for j in idx_list]
            batch.update(static_keys)
            yield batch
        del merged
        buffer.clear()
    ...
    def _convert_batch(self, batch: "pa.RecordBatch") -> Dict[str, Any]:
        ...
        time_context = self._buf_time_context[:B]
        time_context[:] = 0.0
        root_dt = np.asarray(timestamps, dtype=np.int64)
        time_context[:, 0] = ((root_dt + 8 * 3600) // 3600 % 24).astype(np.float32) / 23.0
        time_context[:, 1] = (
        ...
        for domain_idx, domain in enumerate(self.seq_domains):
            ...
            if ts_ci is not None:
                ...
                max_delta = int(BUCKET_BOUNDARIES[-1])
                clipped_diff = np.minimum(time_diff, max_delta)
                log_delta = np.log1p(clipped_diff) / np.log1p(max_delta)
                log_delta[ts_padded == 0] = 0.0
                time_delta[:] = log_delta.astype(np.float32)
                ...
                valid_count = valid_ts.sum(axis=1).astype(np.float32)
                gap = np.maximum(ts_expanded - ts_padded, 0).astype(np.float32)
                gap[~valid_ts] = 0.0
                last_gap = np.zeros(B, dtype=np.float32)
                mean_gap = np.zeros(B, dtype=np.float32)
                rows_with_ts = valid_count > 0
                if rows_with_ts.any():
                    last_pos = np.maximum(valid_count.astype(np.int64) - 1, 0)
                    row_idx = np.arange(B)
                    last_gap[rows_with_ts] = gap[row_idx[rows_with_ts], last_pos[rows_with_ts]]
                ...
                time_context[:, base + 1] = np.log1p(np.minimum(mean_gap, max_delta)) / denom
                time_context[:, base + 2] = (
...
def get_pcvr_data(
    ...
    valid_split_mode: str = 'tail',
    valid_time_window_hours: float = 0.0,
    train_include_valid: bool = False,
    ...
    **kwargs: Any,
    ...
    fraction of Row Groups (in the file order returned by ``glob``). Other
    split modes are available to diagnose train/valid/test distribution drift.
    ``valid_split_mode='time_window'`` uses row-level timestamp filtering:
    validation keeps the last ``valid_time_window_hours`` hours and training
    keeps all earlier samples.
    ``train_include_valid=True`` keeps the validation loader as a monitor but
    lets the training loader scan all Row Groups / timestamps.
    ...
    for f in pq_files:
        pf = pq.ParquetFile(f)
        timestamp_col_idx = pf.schema_arrow.get_field_index('timestamp')
        for i in range(pf.metadata.num_row_groups):
            min_ts, max_ts = _get_row_group_timestamp_range(
                pf, i, timestamp_col_idx)
            rg_info.append((f, i, pf.metadata.row_group(i).num_rows, min_ts, max_ts))
    total_rgs = len(rg_info)

    n_valid_rgs = max(1, int(total_rgs * valid_ratio))
    all_indices = list(range(total_rgs))
    split_mode = valid_split_mode.lower().strip()
    ...
    if split_mode == 'tail':
        valid_indices = all_indices[total_rgs - n_valid_rgs:]
    elif split_mode == 'head':
        valid_indices = all_indices[:n_valid_rgs]
    elif split_mode == 'middle':
        start = max(0, (total_rgs - n_valid_rgs) // 2)
        valid_indices = all_indices[start:start + n_valid_rgs]
    elif split_mode == 'random':
        rng = random.Random(seed)
        valid_indices = sorted(rng.sample(all_indices, n_valid_rgs))
    elif split_mode == 'time_tail':
        def _max_ts(idx: int) -> Tuple[int, int]:
            _, _, _, _, max_ts = rg_info[idx]
            return (int(max_ts) if max_ts is not None else -1, idx)
        valid_indices = sorted(
            sorted(all_indices, key=_max_ts)[total_rgs - n_valid_rgs:]
        )
    elif split_mode == 'time_window':
        if valid_time_window_hours <= 0:
            raise ValueError(
                "valid_split_mode='time_window' requires "
                "--valid_time_window_hours > 0")
        max_ts_values = [int(r[4]) for r in rg_info if r[4] is not None]
        if not max_ts_values:
            raise ValueError("time_window split requires timestamp metadata")
        global_max_ts = max(max_ts_values)
        cutoff_ts = global_max_ts - int(valid_time_window_hours * 3600)
        train_timestamp_max = cutoff_ts
        valid_timestamp_min = cutoff_ts
        # Keep only RowGroups that can contain rows on either side. The final
        # row-level decision is made inside PCVRParquetDataset.
        train_indices = [
            idx for idx, (_, _, _, min_ts, _) in enumerate(rg_info)
            if min_ts is None or int(min_ts) < cutoff_ts
        ]
        valid_indices = [
            idx for idx, (_, _, _, _, max_ts) in enumerate(rg_info)
            if max_ts is None or int(max_ts) >= cutoff_ts
        ]
        logging.info(
            "Sample-level time_window split: global_max_ts=%d, "
            "cutoff_ts=%d, valid_time_window_hours=%.3f",
            global_max_ts, cutoff_ts, valid_time_window_hours)
    else:
        raise ValueError(
            f"Unknown valid_split_mode={valid_split_mode!r}; "
            "expected one of: tail, head, middle, random, time_tail, time_window"
        )
    ...
    if train_include_valid:
        if split_mode == 'time_window':
            ...
            train_indices = all_indices
        logging.info(
            "train_include_valid=True: training loader includes validation "
            "rows; validation metrics are monitors, not strict holdout metrics.")

    # train_ratio: use only the first N% of the training Row Groups.
    if train_ratio < 1.0:
        n_train_rgs = max(1, int(len(train_indices) * train_ratio))
        train_indices = train_indices[:n_train_rgs]
        logging.info(f"train_ratio={train_ratio}: using {n_train_rgs} train Row Groups")

    train_rows = sum(rg_info[idx][2] for idx in train_indices)
    valid_rows = sum(rg_info[idx][2] for idx in valid_indices)

    row_note = "pre-filter rows" if split_mode == 'time_window' else "rows"
    logging.info(
        f"Row Group split ({split_mode}): {len(train_indices)} train "
        f"({train_rows} {row_note}), {len(valid_indices)} valid "
        f"({valid_rows} {row_note}); "
        f"valid index sample={valid_indices[:5]}...{valid_indices[-5:]}"
    )
    _log_rg_time_range("train", rg_info, train_indices)
    _log_rg_time_range("valid", rg_info, valid_indices)
